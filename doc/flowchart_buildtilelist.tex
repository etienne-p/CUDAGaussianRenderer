...
\begin{document}
	\begin{tikzpicture}
		...
		\node (scan-exit) [decision,aspect=2,below=0.5 of scan] {Has pending tiles};
		\node (expand) [process,below=1.0 of scan-exit] {\textbox{Expand tiles to distribute them among threads within the group}};
		...
		\draw [arrow] (scan) -- (scan-exit);
		\draw [arrow] (scan-exit) -- coordinate[midway](m3)(expand);
		...
		\draw [arrow] (scan-exit.west) -- coordinate[midway](m8) + (-3.0,0) |- (m1);
